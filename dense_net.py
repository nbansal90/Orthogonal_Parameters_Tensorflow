...
class DenseNet:
    def __init__(self, data_provider, growth_rate, depth,
                 total_blocks, keep_prob, nesterov_momentum, model_type, dataset,
        ...
        #self.weight_decay = tf.Variable(initial_value=weight_decay, dtype=tf.float32,trainable=False)
        #self.ortho_decay  = ortho_decay
        #self.ortho_decay = tf.Variable(initial_value=ortho_decay, dtype=tf.float32,trainable=False)
    ...
    def l2_reg(self):
        """ Defining the Orthogonal regularizer and return the function at last to be used 
        in Conv layer as kernel regularizer"""
        ...
        def ortho_reg(weights):
                ...
                w = tf.reshape(w, [s[0]*s[1]*s[2], s[3]])

                """ Declaring a Identity Tensor of appropriate size"""
                Ident = tf.Variable(initial_value = np.identity(m[3]),dtype=tf.float32)

                """ Regularizer Wt*W - I """
                wt =  tf.transpose(w)
                mul = tf.matmul(wt,w)
                reg = tf.subtract(mul, Ident)
    ...
    def l2_reg_fully(self):
        """ Defining the Orthogonal regularizer and return the function at last to be used
        in Fully Connected Layer """

        def ortho_reg_fully(weights):
                """ Reshaping the matrix in to 2D tensor for enforcing orthogonality"""
                s = tf.shape(weights)
                m = weights.get_shape()

                """Declaring a Identity Tensor of appropriate size"""
                Ident = tf.Variable(initial_value = np.identity(m[1]),dtype=tf.float32)
                wt =  tf.transpose(weights)
                mul = tf.matmul(wt,weights)
                reg = tf.subtract(mul, Ident)
    ...
    def orthogonal_initializer(self,scale = 1.1):
    	''' From Lasagne and Keras. Reference: Saxe et al., http://arxiv.org/abs/1312.6120
    	'''

    	def initial(shape, dtype=tf.float32,partition_info=None):
    ...
    def log_loss_accuracy_new(self,loss,l2_loss, accuracy, epoch, prefix,
				should_print=True):
    	print ("mean cross_entropy: %f, mean l2_loss: %f, mean accuracy: %f" % (
		loss, l2_loss, accuracy))
    ...
    def _define_inputs(self):
        ...
        self.weight_decay  = tf.placeholder(
            ...
            shape=[],
            name='weight_decay')
        self.ortho_decay  = tf.placeholder(
            tf.float32,
            shape=[],
            name='ortho_decay')
    ...
    def transition_layer_to_classes(self, _input):
        ...
        W = self.weight_variable_orthogonal(
            [features_total, self.n_classes], name='W')
    ...
    def conv2d(self, _input, out_features, kernel_size,
        ...
        kernel = self.weight_variable_msra(
            ...
            name='kernel')
        output = tf.layers.conv2d(_input, out_features, kernel_size, strides=(1,1), padding='SAME',
				kernel_initializer=self.orthogonal_initializer(),kernel_regularizer=self.l2_reg())
        return output
    ...
    def weight_variable_orthogonal(self, shape, name):
        return tf.get_variable(
             name,
             shape=shape,
             initializer=self.orthogonal_initializer(),
             regularizer=self.l2_reg_fully())
    ...
    def _build_graph(self):
        ...
        reg_loss = tf.reduce_sum(reg_ws)


        reg_loss_total = (self.ortho_decay)*reg_loss
        l2_loss_total  = (self.weight_decay)*l2_loss
    ...
    def train_all_epochs(self, train_params):
        ...
        weight_decay = train_params['initial_weight_decay']
        ortho_decay  = train_params['initial_ortho_decay']
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            loss, acc = self.train_one_epoch(
                self.data_provider.train, batch_size, learning_rate,weight_decay,ortho_decay)
    ...
    def train_one_epoch(self, data, batch_size, learning_rate,weight_decay, ortho_decay):
